...
// Go编译配置WebView提供器
class GoBuildConfigProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'goBuildPanel';
    ...
    private async executeCompile() {
        ...
        await executeBuild(command, 'Go项目编译');
    }
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider(GoBuildConfigProvider.viewType, goBuildConfigProvider)
    );
    ...
    const disposables = [
        ...
        // 设置配置值
        vscode.commands.registerCommand('goBuild.setConfigValue', (configKey: string, value: string) => {
...
async function showBuildOptions() {
    const options = [
        {
...
async function buildCurrentFile() {
    ...
    const outputDir = config.get<string>('outputDir', './bin');
    
    await executeBuild(`go build -o ${outputDir}/${fileName} "${filePath}"`, '编译当前文件');
}
...
async function buildProject() {
    ...
    await executeBuild(command, '编译项目');
}
...
async function buildAndRun() {
    ...
    try {
        await executeBuild(buildCommand, '编译项目');
...
async function executeBuild(command: string, taskName: string): Promise<void> {
    outputChannel.clear();
    outputChannel.show();
...
async function crossCompile() {
    ...
    if (selected) {
        ...
        await executeBuild(command, `交叉编译 (${selected.label})`);
    }
}
...
async function openSettings() {
    await vscode.commands.executeCommand('workbench.action.openSettings', 'goBuild');
}



async function executeCompileWithConfig(provider: GoBuildConfigProvider) {
    try {
